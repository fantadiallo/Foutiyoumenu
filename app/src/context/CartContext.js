...
function loadLS() {
  ...
  } catch { return { items: [], note: "", pickupTime: "" }; }
}
...
export function CartProvider({ children }) {
  ...
  const add = p => setItems(prev => {
    ...
    if (i) return prev.map(x => x.id===p.id ? { ...x, qty: x.qty + (p.qty || 1) } : x);
    return [...prev, { id:p.id, name:p.name, price:Number(p.price)||0, qty: p.qty || 1 }];
  });
  ...
  const setQty = (id, qty) =>
    setItems(prev => prev.map(i => i.id===id ? { ...i, qty: Math.max(1, Number(qty)||1) } : i));

  const remove = id => setItems(prev => prev.filter(i => i.id !== id));
  const clear = () => { setItems([]); setNote(""); setPickupTime(""); };

  const count = useMemo(() => items.reduce((s,i)=> s + i.qty, 0), [items]);
  const subtotal = useMemo(() => items.reduce((s,i)=> s + (i.price*i.qty), 0), [items]);

  const value = {
    items, add, dec, setQty, remove, clear,
    count, subtotal,
    note, setNote,
